...
describe("MemoirOS Pipeline Integration", () => {
  ...
  describe("Phase 1: Interview Collection", () => {
    ...
    it("should process user answers and extract entities", async () => {
      ...
      // Process answer
      const answerResponse = await interviewer.processAnswer(
        startResponse.interviewState.interviewId,
        startResponse.nextQuestion!.questionId,
        "我1985年出生在湖北的一个小县城，那时候家里条件挺艰苦的，但大家都很开心。"
      );
    ...
    it("should advance through interview phases", async () => {
      ...
      // Warmup -> Childhood
      const advancedResponse = await interviewer.advancePhase(
        startResponse.interviewState.interviewId
      );
  ...
  describe("Phase 5: Complete Pipeline", () => {
    it("should run the full memoir creation pipeline", async () => {
      ...
      const answerResponse = await interviewer.processAnswer(
        interviewResponse.interviewState.interviewId,
        interviewResponse.nextQuestion!.questionId,
        sampleAnswers[0]
      );
  ...
  describe("Edge Cases and Error Handling", () => {
    ...
    it("should handle LLM failures gracefully", async () => {
      ...
      const failContext: AgentContext = {
        ...mockContext,
        client: {
          provider: "test",
          chat: async () => {
