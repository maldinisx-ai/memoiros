...
describe("FactVerifierAgent", () => {
    ...
    describe("verify()", () => {
        ...
        it("should extract entities from the fact", async () => {
            ...
            const extractEntities = verifier.extractEntities.bind(verifier);
        ...
        it("should determine correct verification strategy", async () => {
            ...
            const determineStrategy = verifier.determineStrategy.bind(verifier);
            const strategy1 = determineStrategy({ fact: "我1990年上了小学" }, { years: [1990], locations: [], events: [] });
            expect(strategy1.type).toBe("era_context");
            // Test targeted search strategy
            const strategy2 = determineStrategy({ fact: "我记得改革开放那一年" }, { years: [], locations: [], events: ["改革开放"] });
