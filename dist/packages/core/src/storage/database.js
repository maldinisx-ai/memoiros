...
import { interviewSaveSchema, questionSaveSchema, answerSaveSchema, timelineEventSaveSchema, voiceProfileSaveSchema, } from "../schemas/database.schemas.js";
import { memcubeItemSaveSchema, memcubeCollectionSaveSchema, } from "../schemas/memcube.schemas.js";
import { memoirSaveSchema, chapterSaveSchema, chapterContentSaveSchema, chapterVersionSaveSchema, } from "../schemas/chapter.schemas.js";
/**
...
export class MemoirOSStorage {
    ...
    saveInterview(interview) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `);
        stmt.run(validated.interviewId, validated.userId, validated.status, validated.startedAt, validated.completedAt ?? null, validated.currentPhase, JSON.stringify(validated.metadata));
    }
    /**
    ...
    loadInterview(interviewId) {
      ...
      WHERE interview_id = ?
    `);
        const row = stmt.get(interviewId);
        if (!row)
            return null;
        return {
    ...
    saveQuestion(question) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(validated.questionId, validated.interviewId, validated.phase, validated.question, validated.questionType, validated.targetEntities ? JSON.stringify(validated.targetEntities) : null, validated.priority, validated.askedAt ?? null, validated.answered ? 1 : 0);
    }
    /**
    ...
    saveAnswer(answer) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(validated.answerId, validated.questionId, validated.interviewId, validated.answer, validated.answeredAt, validated.extractedEntities ? JSON.stringify(validated.extractedEntities) : null, validated.sentiment ?? null, validated.needsFollowup ? 1 : 0, validated.followupTopics ? JSON.stringify(validated.followupTopics) : null);
    }
    /**
    ...
    `);
        const rows = stmt.all(interviewId);
        return rows.map(row => ({
    ...
    saveTimelineEvent(event) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, datetime('now'))
    `);
        stmt.run(validated.eventId, validated.timelineId, validated.userId, validated.date.type, validated.date.year ?? null, validated.date.month ?? null, validated.date.day ?? null, validated.date.era ?? null, validated.date.range ?? null, validated.title, validated.description, validated.category, validated.importance, validated.confidence, validated.tags ? JSON.stringify(validated.tags) : null);
    }
    /**
    ...
    saveVoiceProfile(profile) {
      ...
      VALUES (?, ?, ?, datetime('now'), ?)
    `);
        stmt.run(validated.profileId, validated.userId, JSON.stringify(validated.characteristics), validated.confidence);
    }
    /**
    ...
    saveMemCubeItem(item) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(validated.cubeId, validated.userId, validated.itemId, validated.content, validated.contentHash, validated.itemType, validated.sourceId ?? null, validated.embeddingId ?? null, validated.embeddingModel ?? null, validated.status, validated.createdAt, validated.updatedAt, validated.indexedAt ?? null, JSON.stringify(validated.keywords), JSON.stringify(validated.semanticTags), JSON.stringify(validated.relatedCubeIds), validated.parentCubeId ?? null, validated.accessCount, validated.lastAccessedAt ?? null);
    }
    /**
     * Load MemCube item by ID
     */
    loadMemCubeItem(cubeId) {
        const stmt = this.db.prepare(`
     ...
     */
    updateMemCubeEmbedding(cubeId, embeddingId, embeddingModel) {
        const stmt = this.db.prepare(`
     ...
     */
    updateMemCubeStatus(cubeId, status, indexedAt) {
        const stmt = this.db.prepare(`
      UPDATE memcube_items
     ...
     */
    loadMemCubeItems(userId, status) {
        let query = `
     ...
     */
    saveMemCubeCollection(collection) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(validated.collectionId, validated.userId, validated.name, validated.description ?? null, JSON.stringify(validated.cubeIds), validated.parentCollectionId ?? null, JSON.stringify(validated.metadata), validated.createdAt, validated.updatedAt);
    }
    /**
     * Load MemCube collection by ID
     */
    loadMemCubeCollection(collectionId) {
        const stmt = this.db.prepare(`
    ...
    searchMemCubeFTS(params) {
      ...
      LIMIT ? OFFSET ?
    `);
        const rows = stmt.all(params.userId, ftsQuery, limit, offset);
    ...
    searchInterviewAnswersFTS(params) {
        ...
        const queryParams = [params.userId, params.query];
    ...
    getSearchSuggestions(params) {
        ...
        LIMIT ?
      `);
            const rows = stmt.all(params.userId, `${params.prefix}*`, limit);
     ...
     */
    rebuildInterviewAnswersFTSIndex() {
        this.db.exec(`
     ...
     */
    getFTS5Stats() {
        const memcubeCount = this.db.prepare("SELECT COUNT(*) as count FROM memcube_fts").get();
        const interviewAnswersCount = this.db.prepare("SELECT COUNT(*) as count FROM interview_answers_fts").get();
        const chaptersCount = this.db.prepare("SELECT COUNT(*) as count FROM chapters_fts").get();
        return {
    ...
    `;
        ...
        if (params.memoirId) {
            ...
            queryParams.push(params.memoirId);
        }
        if (params.status) {
            sql += " AND cfts.status = ?";
        ...
        queryParams.push(String(params.limit));
    ...
    getChapterSearchSuggestions(userId, prefix, limit = 10) {
      ...
      LIMIT ?
    `);
        return stmt.all(userId, prefix, limit).map(row => row.suggestion);
    }
    // ============================================
    // Memoir Operations
    // ============================================
    ...
    saveMemoir(memoir) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(validated.memoirId, validated.userId, validated.title, validated.description ?? null, validated.status, JSON.stringify(validated.metadata ?? {}), validated.createdAt, validated.updatedAt);
    }
    /**
     * Load memoir by ID
     */
    loadMemoir(memoirId) {
        const row = this.db.prepare("SELECT * FROM memoirs WHERE memoir_id = ?").get(memoirId);
        if (!row) {
            return null;
        }
        return {
    ...
    saveChapter(chapter) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(validated.chapterId, validated.userId, validated.memoirId, validated.title, validated.status, validated.type, validated.order, validated.createdAt, validated.updatedAt, validated.publishedAt ?? null, JSON.stringify(validated.metadata ?? {}));
    }
    /**
    ...
    saveChapterContent(content) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(validated.contentId, validated.chapterId, validated.content, validated.markdown ?? null, validated.wordCount, validated.characterCount, validated.createdAt, validated.updatedAt);
    }
    /**
     * Load chapter by ID
     */
    loadChapter(chapterId) {
        const row = this.db.prepare("SELECT * FROM chapters WHERE chapter_id = ?").get(chapterId);
        if (!row) {
            return null;
        }
        return {
    ...
    loadChapterContent(chapterId) {
        const row = this.db.prepare("SELECT * FROM chapter_contents WHERE chapter_id = ?").get(chapterId);
        if (!row) {
            return null;
        }
        return {
     ...
     */
    updateChapterStatus(chapterId, status, publishedAt) {
     ...
     */
    updateChapterOrder(chapterId, order) {
        const stmt = this.db.prepare(`
      UPDATE chapters
     ...
     */
    getNextChapterOrder(memoirId) {
        const row = this.db.prepare("SELECT COALESCE(MAX(order_idx), -1) + 1 as next_order FROM chapters WHERE memoir_id = ?").get(memoirId);
    ...
    saveChapterVersion(version) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(validated.versionId, validated.chapterId, validated.versionNumber, validated.content, validated.markdown ?? null, validated.changeType, validated.changeDescription ?? null, validated.createdBy ?? null, validated.createdAt);
    }
    /**
     * Load chapter version by ID
     */
    loadChapterVersion(versionId) {
        const row = this.db.prepare("SELECT * FROM chapter_versions WHERE version_id = ?").get(versionId);
        if (!row) {
            return null;
        }
        return {
     ...
     */
    getLatestChapterVersionNumber(chapterId) {
        const row = this.db.prepare("SELECT COALESCE(MAX(version_number), 0) as max_version FROM chapter_versions WHERE chapter_id = ?").get(chapterId);
        return row.max_version;
    }
    /**
     * Delete chapter versions
     */
    deleteChapterVersions(chapterId) {
     ...
     */
    pruneOldChapterVersions(chapterId, keepCount = 10) {
        this.db.prepare(`
     ...
     */
    exportChapterAsMarkdown(chapterId) {
     ...
     */
    exportMemoirAsMarkdown(memoirId) {
    ...
    `);
        const interviews = interviewsStmt.all(userId);
