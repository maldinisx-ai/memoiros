...
export class MemoirWriterAgent extends BaseAgent {
    ...
    determineChapterPeriod(profile, chapterNumber, focusPeriod) {
        // 如果指定了关注时间段，使用它
    ...
    filterEventsByPeriod(timeline, period) {
    ...
    buildChapterContext(profile, events, period) {
...
${eventsList || "暂无"}`;
    ...
    buildSystemPrompt(profile, targetWords) {
...
(正文内容)`;
    ...
    buildUserPrompt(chapterNumber, context, events, period) {
...
${eventsSummary}`;
    ...
    parseOutput(content, chapterNumber) {
        const extract = (tag) => {
    ...
    extractYearFromString(dateString) {
        const match = dateString.match(/(\d{4})/);
        return match ? parseInt(match[1], 10) : null;
    }
    extractYearFromEvent(date) {
...
//# sourceMappingURL=memoir-writer.js.map