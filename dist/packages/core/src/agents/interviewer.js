...
import { createContextManager } from "../utils/context-manager.js";
/**
...
export class InterviewerAgent extends BaseAgent {
     ...
     */
    async processAnswer(interviewId, questionId, answer, options) {
     ...
     * Move to next phase
     */
    async advancePhase(interviewId) {
        const interviewState = await this.loadInterviewState(interviewId);
        const phases = [
     ...
     */
    async generateNextQuestion(state, options) {
     ...
     */
    buildQuestionPrompt(state, template, options) {
        const contextManager = this.getContextManager(state.interviewId);
...
输出格式（JSON）：
{
  ...
  "missingEntities": ["year", "location"]  // 缺失但有用的实体类型
     ...
     * Extract facts from answer
     */
    async extractFacts(answer, entities) {
...
输出格式（JSON）：
{
  "facts": [
     ...
     */
    async getOrCreateInterviewState(request) {
     ...
     */
    async loadInterviewState(interviewId) {
        if (!this.storage) {
     ...
     */
    async updateInterviewState(state, answer) {
        const updated = {
            ...state,
            answers: [...state.answers, answer],
            questions: state.questions.map(q => q.questionId === answer.questionId
     ...
     */
    async saveInterviewState(state) {
        if (!this.storage) {
        ...
        for (const answer of state.answers) {
            this.storage.saveAnswer({
                ...
                extractedEntities: answer.extractedEntities,
     ...
     */
    getFallbackQuestions(state, count) {
        const fallbacks = {
            warmup: [
