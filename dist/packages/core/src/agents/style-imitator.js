...
export class StyleImitatorAgent extends BaseAgent {
     ...
     */
    async createVoiceProfile(request) {
        this.log?.info(`[StyleImitator] Creating voice profile for user: ${request.userId}`);
        // Step 1: Analyze each sample
        const sampleAnalyses = await Promise.all(request.samples.map(sample => this.analyzeSample(sample)));
...
输出格式（JSON）：
{
  "characteristics": {
     ...
     */
    aggregateCharacteristics(analyses) {
        ...
        const avgSentenceLength = this.average(validAnalyses.map(a => a.characteristics.avgSentenceLength ?? defaults.avgSentenceLength));
        // Most frequent categorical values
        const sentenceComplexity = this.mostFrequent(validAnalyses.map(a => a.characteristics.sentenceComplexity ?? defaults.sentenceComplexity)) ?? defaults.sentenceComplexity;
        const prefersShortSentences = this.mode(validAnalyses.map(a => a.characteristics.prefersShortSentences ?? defaults.prefersShortSentences)) ?? defaults.prefersShortSentences;
        const vocabularyLevel = this.mostFrequent(validAnalyses.map(a => a.characteristics.vocabularyLevel ?? defaults.vocabularyLevel)) ?? defaults.vocabularyLevel;
        // Aggregate arrays (remove duplicates, keep frequency)
        const commonWords = this.aggregateStringArrays(validAnalyses.map(a => a.characteristics.commonWords ?? []));
        const idiosyncraticPhrases = this.aggregateStringArrays(validAnalyses.map(a => a.characteristics.idiosyncraticPhrases ?? []));
        const dominantTone = this.aggregateStringArrays(validAnalyses.map(a => a.characteristics.dominantTone ?? []));
        const emotionalRange = this.mostFrequent(validAnalyses.map(a => a.characteristics.emotionalRange ?? defaults.emotionalRange)) ?? defaults.emotionalRange;
        const usesEllipsis = this.mode(validAnalyses.map(a => a.characteristics.usesEllipsis ?? defaults.usesEllipsis)) ?? defaults.usesEllipsis;
        const usesEmDash = this.mode(validAnalyses.map(a => a.characteristics.usesEmDash ?? defaults.usesEmDash)) ?? defaults.usesEmDash;
        const exclamationFrequency = this.mostFrequent(validAnalyses.map(a => a.characteristics.exclamationFrequency ?? defaults.exclamationFrequency)) ?? defaults.exclamationFrequency;
        const dialect = this.aggregateStringArrays(validAnalyses.map(a => a.characteristics.dialect ?? []));
        const eraMarkers = this.aggregateStringArrays(validAnalyses.map(a => a.characteristics.eraMarkers ?? []));
        const perspective = this.mostFrequent(validAnalyses.map(a => a.characteristics.perspective ?? defaults.perspective)) ?? defaults.perspective;
        const tense = this.mostFrequent(validAnalyses.map(a => a.characteristics.tense ?? defaults.tense)) ?? defaults.tense;
        const detailLevel = this.mostFrequent(validAnalyses.map(a => a.characteristics.detailLevel ?? defaults.detailLevel)) ?? defaults.detailLevel;
        return {
            avgSentenceLength,
            sentenceComplexity,
     ...
     */
    calculateProfileConfidence(analyses) {
        const validAnalyses = analyses.filter(a => a.confidence > 0);
        if (validAnalyses.length === 0)
            return 0;
     ...
     */
    buildStylePrompt(sourceText, profile, options) {
...
请改写以下文字，输出格式（JSON）：
{
  ...
  "confidence": 0.9
     ...
     */
    adjustIntensity(original, rewritten, intensity) {
        if (intensity === "strong") {
            return rewritten;
        }
     ...
     */
    extractDominantTraits(profile) {
        const c = profile.characteristics;
        const traits = [];
        if (c.prefersShortSentences)
            traits.push("偏好短句");
        if (c.usesEllipsis)
            traits.push("常用省略号");
        if (c.emotionalRange === "expressive")
            traits.push("情感丰富");
        if (c.emotionalRange === "restricted")
            traits.push("情感克制");
        if (c.detailLevel === "rich")
            traits.push("细节丰富");
        if (c.detailLevel === "sparse")
            traits.push("言简意赅");
        return traits;
    }
    /**
     ...
     */
    async generateVoiceSuggestions(profile) {
        const suggestions = [];
        const c = profile.characteristics;
...
//# sourceMappingURL=style-imitator.js.map