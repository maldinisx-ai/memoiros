...
export class PreprocessorAgent extends BaseAgent {
    storage; // FileStorage or MemoirOSStorage
     ...
     */
    async loadInterviewData(userId, interviewId) {
     ...
     */
    async extractBasicInfo(interviewData) {
...
返回 JSON 格式：
{
  ...
  "occupation": "职业"
     ...
     */
    async extractEntities(interviewData) {
        const yearsSet = new Set();
        const locationsSet = new Set();
        const peopleSet = new Set();
        const eventsSet = new Set();
        const emotionsSet = new Set();
        const missingEntitiesSet = new Set();
        for (const answer of interviewData.answers) {
            if (answer.extractedEntities) {
                const entities = answer.extractedEntities;
                if (entities.years)
                    entities.years.forEach(y => yearsSet.add(y));
                if (entities.locations)
                    entities.locations.forEach(l => locationsSet.add(l));
                if (entities.people)
                    entities.people.forEach(p => peopleSet.add(p));
                if (entities.events)
                    entities.events.forEach(e => eventsSet.add(e));
                if (entities.emotions)
                    entities.emotions.forEach(e => emotionsSet.add(e));
            }
        }
        return {
     ...
     */
    async extractFacts(interviewData) {
...
返回 JSON 格式：
{
  "facts": [
     ...
     */
    async buildTimeline(interviewData, entities) {
        try {
...
请按时间顺序排列事件，返回 JSON 格式：
{
  "events": [
     ...
     */
    async analyzeVoiceProfile(answers) {
     ...
     */
    async extractThemes(interviewData, timeline) {
        try {
...
返回 JSON 格式：
{
  "themes": [
     ...
     */
    async buildStoryStructure(interviewData, timeline) {
        try {
...
返回 JSON 格式：
{
  ...
  "resolution": {
    ...
    "emotionalArc": "情感弧线"
            ...
            return StoryStructureSchema.parse(parsed);
        }
        catch (error) {
     ...
     * Generate metadata
     */
    generateMetadata(interviewData, entities, facts) {
        const gaps = [];
        ...
        const confidence = Math.min(1, (facts.length * 0.1) +
