...
import type { UserProfile, Theme, StoryPhase } from "./preprocessor.js";
...
export class MemoirArchitectAgent extends BaseAgent {
  ...
  async generateOutline(request: MemoirOutlineRequest): Promise<MemoirOutlineOutput> {
  ...
  private determineBestStructure(profile: UserProfile): "chronological" | "thematic" | "mixed" {
    // 如果有明确的时间线，优先用时间顺序
  ...
  private buildContext(profile: UserProfile): string {
...
${timelinePreview || "暂无"}`;
  ...
  private buildSystemPrompt(structure: "chronological" | "thematic" | "mixed", targetChapters: number): string {
    const structureDesc = {
...
（重复以上格式，列出所有章节）`;
  ...
  private buildUserPrompt(context: string, targetChapters: number, structure: string): string {
...
5. 关键事件要覆盖，但不必面面俱到`;
  ...
  private parseOutput(content: string, profile: UserProfile, targetChapters: number, actualStructure: "chronological" | "thematic" | "mixed"): MemoirOutlineOutput {
  ...
  private parseChapterOutline(raw: string, targetChapters: number): ReadonlyArray<ChapterOutline> {
  ...
  private parsePeriod(raw: string): { start: string; end: string } {
    // 尝试解析 "XXXX年 - XXXX年" 格式
  ...
  private generateDefaultOutline(targetChapters: number): ReadonlyArray<ChapterOutline> {
    const chapters: ChapterOutline[] = [];
