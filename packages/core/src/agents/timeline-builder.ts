...
export class TimelineBuilderAgent extends BaseAgent {
   ...
   */
  async buildTimeline(request: TimelineBuildRequest): Promise<TimelineBuildResult> {
   ...
   */
  private async extractEventsFromAnswers(
   ...
   */
  private async extractEventsFromAnswer(
    answer: string,
...
- approximate: 近似年份（如"大约1990年"、"90年代初"）`;
   ...
   * Sort events by date
   */
  private sortEventsByDate(events: ReadonlyArray<TimelineEvent>): ReadonlyArray<TimelineEvent> {
   ...
   */
  private extractYear(date: TimelineDate): number {
    switch (date.type) {
      case "exact":
        return date.year ?? 0;
      case "approximate":
   ...
   */
  private mergeEvents(
    newEvents: ReadonlyArray<TimelineEvent>,
    existingEvents: ReadonlyArray<TimelineEvent>
    ...
    for (const newEvent of newEvents) {
      ...
      const similarIndex = merged.findIndex(e =>
        this.areEventsSimilar(e, newEvent)
      );
   ...
   */
  private areEventsSimilar(a: TimelineEvent, b: TimelineEvent): boolean {
  ...
  ): Promise<ReadonlyArray<TimelineConflict>> {
   ...
   */
  private areEventsConflicting(a: TimelineEvent, b: TimelineEvent): boolean {
  ...
  private async detectLLMConflicts(
    ...
    const eventsSummary = events.map(e =>
      `- [${e.category}] ${e.title} (${this.formatDate(e.date)})`
    ).join("\n");
...
输出格式（JSON）：
{
  ...
  private async identifyGaps(
    events: ReadonlyArray<TimelineEvent>
  ): Promise<ReadonlyArray<TimelineGap>> {
    if (events.length < 2) {
      return [];
    }
   ...
   */
  private async generateGapQuestions(startYear: number, endYear: number): Promise<ReadonlyArray<string>> {
...
输出格式（JSON）：
{
  ...
  private buildMetadata(
    events: ReadonlyArray<TimelineEvent>,
    conflicts: ReadonlyArray<TimelineConflict>,
    gaps: ReadonlyArray<TimelineGap>
  ): TimelineMetadata & { readonly earliestYear: number; readonly latestYear: number } {
   ...
   */
  private extractThemes(events: ReadonlyArray<TimelineEvent>): ReadonlyArray<string> {
   ...
   */
  private determineLifeStage(year: number, birthYear: number): string {
   ...
   */
  private formatDate(date: TimelineDate): string {
    switch (date.type) {
      case "exact":
   ...
   * Generate summary
   */
  private generateSummary(timeline: Timeline): string {
