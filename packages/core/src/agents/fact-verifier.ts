...
export class FactVerifierAgent extends BaseAgent {
  private readonly browseClient: BrowseClient | null;
   ...
   */
  async verify(request: FactVerificationRequest): Promise<FactVerificationResult> {
   ...
   */
  private extractEntities(fact: string): ExtractedEntities {
  ...
  ): { type: string; sources: ReadonlyArray<string> } {
   ...
   */
  private buildSourcesForEvents(events: ReadonlyArray<string>): ReadonlyArray<string> {
   ...
   */
  private buildSourcesForEra(year: number): ReadonlyArray<string> {
  ...
  private async executeVerification(
    request: FactVerificationRequest,
    entities: ExtractedEntities,
    strategy: { type: string; sources: ReadonlyArray<string> }
...
${entities ? `提取的实体：${JSON.stringify(entities)}` : ""}`;
   ...
   */
  private async verifyWithBrowse(
    request: FactVerificationRequest,
    entities: ExtractedEntities,
    sources: ReadonlyArray<string>
  ): Promise<Partial<FactVerificationResult>> {
    if (!this.browseClient) {
      return { sources: [] };
    }

    const searchResults = await this.browseClient.searchAndExtract(
      request.fact.slice(0, 50), // Use first 50 chars as search query
      sources
    );
...
输出格式（JSON）：
{
  ...
  private combineResults(
    llmResult: FactVerificationResult,
    browseResult: Partial<FactVerificationResult>
  ): FactVerificationResult {
   ...
   */
  private parseVerificationResult(content: string, fact: string): FactVerificationResult {
    try {
      // Try to extract JSON
