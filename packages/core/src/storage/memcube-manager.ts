...
import {
  memcubeItemSchema,
  memcubeCollectionSchema,
  memcubeQuerySchema,
  memcubeSearchResultSchema,
  type MemCubeItem,
  type MemCubeCollection,
  type MemCubeQuery,
  type MemCubeSearchResult,
  type MemCubeStatus,
...
export class MemCubeManager {
  ...
  private readonly embeddingCache: Map<string, readonly number[]> = new Map();

  constructor(
    storage: MemoirOSStorage,
  ...
  async createItem(params: {
    ...
    const item: MemCubeItem = memcubeItemSchema.parse({
      cubeId,
   ...
   */
  async indexItem(cubeId: string): Promise<void> {
    ...
    try {
      ...
      this.storage.updateMemCubeEmbedding(
        cubeId,
        embeddingId,
        "default" // In production, this would be the actual model name
      );
   ...
   */
  async indexBatch(cubeIds: ReadonlyArray<string>): Promise<void> {
    const items = cubeIds
    ...
    try {
      ...
      for (let i = 0; i < items.length; i++) {
        ...
        this.storage.updateMemCubeStatus(
          items[i]!.cubeId,
          "indexed",
  ...
  async semanticSearch(
    ...
    const results: Array<{ item: typeof items[number]; score: number }> = [];
    for (const item of items) {
      if (item.embeddingId) {
    ...
    const searchResults: MemCubeSearchResult[] = [];
  ...
  addToCollection(collectionId: string, cubeIds: ReadonlyArray<string>): void {
    ...
    const updatedCubeIds = Array.from(
      new Set([...collection.cubeIds, ...cubeIds])
    );
  ...
  linkItems(cubeId: string, relatedCubeIds: ReadonlyArray<string>): void {
    ...
    for (const relatedId of relatedCubeIds) {
      ...
      if (relatedItem) {
        ...
        const relatedItemSave: MemCubeItemSave = {
          ...
          itemType: relatedItem.itemType as MemCubeItem["itemType"],
    ...
    // Convert to MemCubeItemSave format
    const itemSave: MemCubeItemSave = {
      cubeId: item.cubeId,
      userId: item.userId,
      itemId: item.itemId,
      content: item.content,
      contentHash: item.contentHash,
      itemType: item.itemType as MemCubeItem["itemType"],
  ...
  getItems(
    ...
    return items.map(item => ({
      ...
      embeddingId: item.embeddingId,
      embeddingModel: undefined,
      status: item.status as MemCubeStatus,
      createdAt: "",
      updatedAt: "",
      indexedAt: undefined,
      keywords: [],
  ...
  private findItemByContentHash(userId: string, contentHash: string): MemCubeItem | null {
  ...
  private findRelatedItems(cubeId: string): ReadonlyArray<{
    cubeId: string;
    score: number;
  }> {
    const item = this.storage.loadMemCubeItem(cubeId);
  ...
  async hybridSearch(params: {
    ...
    semanticWeight?: number; // Weight for semantic results (default: 0.5)
  }): Promise<ReadonlyArray<MemCubeSearchResult>> {
    ...
    // Build final results
    const results: MemCubeSearchResult[] = [];
